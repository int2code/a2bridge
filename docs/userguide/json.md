...
When a middle node receives both downstream and upstream slots locally, the local TX frame buffer is filled with received downstream slots first, followed by received upstream slots. This means upstream slots selected by `UpstreamReceiveSlots` appear on local DTX channels after any locally received downstream slots.

#### DownstreamReceiveSlots
Optional array of 0-based downstream slot indexes received locally by this node from its A-port and transmitted on the local I2S/TDM DTX pins. A non-empty array programs `A2B_DNMASK0` through `A2B_DNMASK3` and enables `A2B_LDNSLOTS.DNMASKEN`.
When a middle node receives both downstream and upstream slots locally, downstream slots selected by `DownstreamReceiveSlots` are placed first in the local TX frame buffer. Any upstream slots selected by `UpstreamReceiveSlots` are placed after them.

#### DownstreamMaskMode
Optional `"True"` or `"False"` value. When set to `"True"`, the driver enables `A2B_LDNSLOTS.DNMASKEN` even if `DownstreamReceiveSlots` is empty. This allows a middle node to contribute local downstream slots using `LocalDnSlots` without locally consuming any incoming downstream slots.
...
- If downstream slots are also received locally, these upstream slots are placed in the local TX frame buffer after the locally received downstream slots.
...
- Locally received downstream slots are placed first in the local TX frame buffer, before any locally received upstream slots.
- Because `DownstreamReceiveSlots` is not empty, the driver enables `LDNSLOTS.DNMASKEN`. The same mode can also be enabled explicitly with `DownstreamMaskMode: "True"`. In this mode, `LocalDnSlots` no longer means "consume this many downstream slots"; it means "contribute this many local downstream slots".
